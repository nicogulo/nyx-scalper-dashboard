            ...
            displaySignals.length > 0 ? (
                  ...
                  <div className="flex items-center gap-3 text-xs">
                    ...
                    {sig.tp && (
                      <span className="text-emerald-400">
                        TP: {formatPrice(sig.tp, sig.symbol)}
                      </span>
                    )}
                    {sig.sl && (
                      <span className="text-red-400">
                        SL: {formatPrice(sig.sl, sig.symbol)}
                      </span>
                    )}
                    {sig.rsi_5 !== undefined && (
                      <span className="text-slate-400">RSI: {sig.rsi_5}</span>
                    )}
                    {sig.vol_ratio !== undefined && (
                      <span className="text-slate-400">Vol: {sig.vol_ratio}x</span>
                    )}
                  </div>
                  <div className="mt-1.5 text-[11px] text-slate-500 leading-relaxed">
                    <span className="text-purple-400 font-medium">[{sig.strategy}]</span>{" "}
                    {sig.reason}
                  </div>
                </motion.div>
              ))
              ...
              >
                ...
                <p className="text-slate-600 text-xs mt-1">
                  Signals will appear here when a strategy triggers
                </p>
              </motion.div>
            )
          )}
